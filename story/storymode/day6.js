...
function playMusic(difficulty) {
    ...
    let musicId = 'hard-music'; // 무조건 HARD 난이도의 음악 재생
...
document.querySelectorAll('.difficulty-btn').forEach(button => {
    button.addEventListener('click', function() {
        startGame(parseInt(this.dataset.time)); // HARD 난이도는 이미 data-time="1500"으로 설정됨
    });
});
...
function abruptZoomEffect() {
    const gameContainer = document.getElementById('game-container');
    const scales = [1.2, 1.4, 0.8, 0.6]; // Define abrupt zoom in and out scales
    const randomInterval = Math.floor(Math.random() * 8000) + 3000; // Random interval between 3 to 11 seconds

    setTimeout(() => {
        const randomScale = scales[Math.floor(Math.random() * scales.length)];
        gameContainer.style.transform = `scale(${randomScale})`;

        setTimeout(() => {
            gameContainer.style.transform = 'scale(1)'; // Instantly revert to normal
        }, 200); // Keep the zoom for a short, abrupt period

        abruptZoomEffect(); // Repeat the effect
    }, randomInterval);
}
